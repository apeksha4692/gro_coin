...
//BSC Wallet generate without metamask
    public function bsc_generate_wallet(){
        $bsc_api_key = $this->config->item('bsc_api_key');
    ...
    public function bsc_account($xpub){
        $bsc_api_key = $this->config->item('bsc_api_key');

        $curl = curl_init();

        curl_setopt_array($curl, [
          CURLOPT_URL => "https://api-us-west1.tatum.io/v3/bsc/address/".$xpub."/1",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
    ...
    public function bsc_private_key($mnemonic){
        $bsc_api_key = $this->config->item('bsc_api_key');
        ...
        curl_setopt_array($curl, [
          ...
          CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
          CURLOPT_CUSTOMREQUEST => "POST",
          CURLOPT_POSTFIELDS => "{\"index\":1,\"mnemonic\":\"$mnemonic\"}",
          CURLOPT_HTTPHEADER => [
            "content-type: application/json",
            "x-api-key:".$bsc_api_key
          ],
    ...
    public function wallet_balance($user_wallet_address){
        $bsc_api_key = $this->config->item('bsc_api_key');

        $curl = curl_init();

        curl_setopt_array($curl, [
          CURLOPT_URL => "https://api-us-west1.tatum.io/v3/bsc/account/balance/".$user_wallet_address,
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
    ...
    public function transfer_amount($fromPrivateKey,$to_address,$amount){
        $bsc_api_key = $this->config->item('bsc_api_key');

        $curl = curl_init();

        curl_setopt_array($curl, [
          CURLOPT_URL => "https://api-us-west1.tatum.io/v3/bsc/transaction",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => "",
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
          CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
          CURLOPT_CUSTOMREQUEST => "POST",
          CURLOPT_POSTFIELDS => "{\"to\":\"$to_address\",\"currency\":\"BSC\",\"amount\":\"$amount\",\"fromPrivateKey\":\"$fromPrivateKey\"}",
          CURLOPT_HTTPHEADER => [
            "content-type: application/json",
            "x-api-key:".$bsc_api_key
          ],
